...
export function ProductImagesPage({ productId }: ProductImagesPage) {
		...
		onSubmit: async ({ value }) => {
			if (value.images.length > MAX_IMAGES_PER_PRODUCT) {
				ToastService.warning(
					`No máximo ${MAX_IMAGES_PER_PRODUCT} imagens por produto.`,
				);
				return;
			}
	...
	}, [productData, productImagesForm.setFieldValue]);
					...
					>
						...
						>
							...
							<span className="text-muted-foreground text-xs">
								Máximo de {MAX_IMAGES_PER_PRODUCT} imagens por produto e 3MB por
								imagem
							</span>
						</div>
